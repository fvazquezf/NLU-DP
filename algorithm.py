...
class Sample(object):
    ...
    def state_to_feats(self, nbuffer_feats: int = 2, nstack_feats: int = 2):
        ...
        """
        def get_word_upos(token):
            return (token.form, token.upos) if token else ("<PAD>", "<PAD>")

        # Extract stack features
        stack_tokens = [self._state.S[-i-1] if len(self._state.S) > i else None for i in range(nstack_feats)]
        buffer_tokens = [self._state.B[i] if len(self._state.B) > i else None for i in range(nbuffer_feats)]

        stack_feats = [get_word_upos(tok)[0] for tok in stack_tokens]
        buffer_feats = [get_word_upos(tok)[0] for tok in buffer_tokens]

        stack_upos = [get_word_upos(tok)[1] for tok in stack_tokens]
        buffer_upos = [get_word_upos(tok)[1] for tok in buffer_tokens]

        return stack_feats + buffer_feats + stack_upos + buffer_upos
...
class ArcEager():
    ...
    def LA_is_valid(self, state: State) -> bool:
        ...
        if not state.S or not state.B:
            return False
        s = state.S[-1]
        if s.id == 0:  # ROOT cannot be dependent
            return False
        # The dependent cannot already have a head
        if any(arc[2] == s.id for arc in state.A):
            return False
        return True
    ...
    def RA_is_valid(self, state: State) -> bool:
        ...
        """
        return bool(state.S and state.B)
    ...
    def REDUCE_is_correct(self, state: State) -> bool:
        ...
        if not self.REDUCE_is_valid(state):
            return False
        s = state.S[-1]
        # No word in buffer whose head is s.id
        return all(tok.head != s.id for tok in state.B)
    ...
    def REDUCE_is_valid(self, state: State) -> bool:
        ...
        """
        return bool(state.S and any(arc[2] == state.S[-1].id for arc in state.A))
    ...
    def apply_transition(self, state: State, transition: Transition):
        ...
        if t == self.LA and self.LA_is_valid(state):
            ...
            state.S.pop()  # remove dependent from stack
